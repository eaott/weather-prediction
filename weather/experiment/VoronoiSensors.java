...
public class VoronoiSensors {
	...
	//		System.out.println("map generated -- " + data.length + " " + data[0].length);
		...
		Sensor[] updated = new Sensor[used.size()];
		int index = 0;
		for (int usedSensor : used)
		{
			System.out.println(sensorArr[usedSensor].name);
			updated[index++] = sensorArr[usedSensor];
		}
		
		// Sensor[] updated has all the required sensors for this radar.
	}
}
